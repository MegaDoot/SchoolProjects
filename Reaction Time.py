...
class Table(tk.Frame):
    def __init__(self, master, rows, columns):
        ...
        self.array = [
            [FrameLbl(
                self, 180, 50, text="", relief="solid",
                borderwidth=3, **style(15)) for _ in range(columns)
                ] for _ in range(rows)
                ]
...
class MainFrame(tk.Frame):
    def __init__(self, master, **kwargs):
        ...
        tk.Label(self, text="Test your reaction times!", **style(50)).grid(
            row=0, column=0)
...
and only your scores will be recorded for others to compete against!""",
                   width=1500, justify="center", **style(20)).grid(
                       row=1, column=0)
        self.start_btn = tk.Button(self, text="Begin!", state="normal", command=self.start, **style(30))
        self.start_btn.grid(row=2, column=0, pady=10)
        self.colour = tk.Frame(self, bg="red", width=200, height=200)
